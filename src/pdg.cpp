                                              ...
                                              int inABranch)
{
			...
			for (auto& def : stmt.first->getDefine())
			{
				...
				if (def_map.find(def) != def_map.end())
				{
					for (auto defStmt = def_map[def].begin(); defStmt != def_map[def].end();)
					{
						...
						// if we are in the same branch && we have a def-def relation, overwrite the previous def.
						if (inABranch > 0)
						{
	...
	if (name() == Type::Loop)
	{
		loopRefs.erase(std::remove(loopRefs.begin(), loopRefs.end(), this), loopRefs.end());
	}
	if (name() == Type::Branch)
	{
		inABranch--;
...
std::string Statement::dumpLevel(int level)
{
	...
	auto ret = nameAsString() + nodeId + defs + uses + locs + dataDeps + cparents + dparents + "\n";
...
{
	// (T, U) => "T,,"
	auto text = clang::Lexer::getSourceText(clang::CharSourceRange::getTokenRange(s->getSourceRange()),
	                                        sm, clang::LangOptions(), nullptr).str();
