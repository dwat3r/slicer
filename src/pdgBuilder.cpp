...
namespace clang
{
	namespace slicer
	{
		...
		void PDGBuilder::setSlicingStmt(const MatchFinder::MatchResult& result, const Stmt* astRef)
		{
		...
		void PDGBuilder::registerMatchers(MatchFinder* MatchFinder)
		{
			...
			// assign
			const auto decls = declStmt(forEachDescendant(varDecl(anyOf(
			...
			const auto bop = binaryOperator(isAssignmentOp(),
			...
			const auto uop = unaryOperator(isIncrementDecrementOp(),
			                               hasDescendant(declRefExpr(to(varDecl().bind("uval"))))).bind("unop");

			const auto ret = returnStmt(forEachDescendant(declRefExpr(to(varDecl().bind("retVar"))))).bind("ret");
			// branch
			const auto ifs = ifStmt(hasCondition(anyOf(
			...
			const auto whiles = whileStmt(hasCondition(anyOf(
		...
		void PDGBuilder::onEndOfTranslationUnit()
		{
			...
			{
				llvm::errs() << "You've given invalid location for slicing var, since I've found no variable there.\n";
				return;
			}
